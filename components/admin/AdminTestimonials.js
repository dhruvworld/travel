...
exports.default = AdminTestimonials;
...
function AdminTestimonials() {
    ...
    var _c = (0, react_1.useState)({
        ...
        stars: 5,
        image: ''
    }), newTestimonial = _c[0], setNewTestimonial = _c[1];
            ...
            switch (_a.label) {
                ...
                case 2:
                    ...
                    setTestimonials(testimonials.map(function (t) {
                        return t.id === id ? updatedTestimonial_1 : t;
                    }));
    ...
    return (<section className="p-4 space-y-6">
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" name="name" value={newTestimonial.name} onChange={handleInputChange} className="w-full p-2 border rounded" placeholder="Customer name"/>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Country</label>
              <input type="text" name="country" value={newTestimonial.country} onChange={handleInputChange} className="w-full p-2 border rounded" placeholder="e.g. India"/>
            </div>
          </div>
          
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Feedback *</label>
            <textarea name="feedback" value={newTestimonial.feedback} onChange={handleInputChange} rows={3} className="w-full p-2 border rounded" placeholder="Customer feedback"></textarea>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="text" name="image" value={newTestimonial.image} onChange={handleInputChange} className="w-full p-2 border rounded" placeholder="https://example.com/image.jpg"/>
            </div>
          </div>
          
          <button onClick={handleAddTestimonial} className="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">
            Add Testimonial
          </button>
        </div>
      </div>
      ...
      {loading ? (<div className="text-center py-8 text-gray-500">Loading testimonials...</div>) : (<div className="space-y-4">
          {testimonials.length > 0 ? (testimonials.map(function (testimonial) { return (<div key={testimonial.id} className={"border p-4 rounded-lg shadow bg-white ".concat(!testimonial.active ? 'opacity-70' : '')}>
                <div className="flex justify-between">
                  ...
                  <div className="space-x-2">
                    ...
                    <button onClick={function () { return handleDelete(testimonial.id); }} className="px-3 py-1 rounded bg-red-600 text-white">
                      Delete
                    </button>
                  </div>
                </div>
                <p className="mt-3 text-gray-700 italic">"{testimonial.feedback}"</p>
              </div>); })) : (<div className="text-center py-8 text-gray-500">No testimonials found. Add one above!</div>)}
        </div>)}
    </section>);
}
